...
public class withdraw extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		phone=findViewById(R.id.withdrawphone);
		code=findViewById(R.id.withdrawcode);
		check=findViewById(R.id.withdrawbutton);
	...
	private void checkdetails(final String pphone, final String ccode)
	{
		...
		reference1.addListenerForSingleValueEvent(new ValueEventListener() {
			...
			public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
				if (dataSnapshot.child("withdrawals").child(pphone).exists()){

					persondetails details= dataSnapshot.child("withdrawals").child(pphone).getValue(persondetails.class);

					if (details.getPhone().equals(pphone))
					{
						if (details.getCode().equals(ccode))
						{
							Toast.makeText(withdraw.this, "details exist...", Toast.LENGTH_SHORT).show();
							loadingBar.dismiss();
						}
						else {
							Toast.makeText(withdraw.this, "The code does not exist", Toast.LENGTH_SHORT).show();
							loadingBar.dismiss();
						}
					}
				}
				else {
					Toast.makeText(withdraw.this, "Phone number doesnt exist", Toast.LENGTH_SHORT).show();
					loadingBar.dismiss();
				}
			}
			@Override
